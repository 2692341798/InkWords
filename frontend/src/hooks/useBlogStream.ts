...
export const useBlogStream = () => {
  ...
  const generateSingle = useCallback(async (sourceContent: string) => {
    ...
    const flushUpdates = () => {
      if (pendingUpdates) {
        store.appendGeneratedContent(pendingUpdates)
        pendingUpdates = ''
      }
      if (updateTimeout) {
  ...
  const generateSeries = useCallback(async () => {
    ...
    const flushUpdates = () => {
      if (Object.keys(pendingUpdates).length > 0) {
        store.appendChapterContents(pendingUpdates)
        pendingUpdates = {}
      }
      if (updateTimeout) {
        clearTimeout(updateTimeout)
          ...
          if (msg.event === 'chunk') {
            try {
              ...
              if (data.status === 'generating') {
                ...
                pendingUpdates[data.chapter_sort] = (pendingUpdates[data.chapter_sort] || '') + data.content
                if (!updateTimeout) {
                  updateTimeout = setTimeout(flushUpdates, 200) // Buffer updates for 200ms
                }
                ...
                flushUpdates()
                store.updateChapterStatus(data.chapter_sort, 'error')
                store.appendChapterContent(data.chapter_sort, '\n\n**生成失败**：' + (data.message || '未知错误'))
              }
            } catch {
              // Ignore parse error
            }
