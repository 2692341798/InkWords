...
## 1. 目标与背景
目前在 InkWords（墨言知识训练平台）中，如果解析一个已解析过的 GitHub 仓库（且源码有更新），系统虽然能够对比出哪些章节需要 `skip`、哪些需要 `regenerate`，但在处理 `regenerate` 章节时，LLM 会完全从头生成新的博客内容，没有利用该章节旧版本的内容作为上下文。这不仅浪费了 Token 资源，还可能导致旧版本中沉淀的优秀解释或行文风格丢失。
