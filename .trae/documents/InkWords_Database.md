...
- `parent_id` (BIGINT/UUID, Nullable): **父级系列 ID**。若为大项目生成的系列博客，此字段指向代表“整个系列”的根节点记录（系统会在开始生成系列前**主动创建并持久化该根节点**，其 `title` 采用 AI 提取并经用户确认的系列标题，避免其下子节点成为无法被查询的孤岛数据。在断点续传时，复用此 ID 将新生成的章节挂载到同一个系列下）。若是单篇独立博客，则为 NULL。
