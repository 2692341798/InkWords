...
- `generate_single` 兼容说明：
  - 推荐新契约仍为 `payload.source_content + payload.source_type`。
  - 为兼容微服务化前后的历史调用方，当 `payload.source_type=topic` 且 `payload.source_content` 为空时，后端会自动回退读取 `payload.topic` 作为真实生成源内容。
