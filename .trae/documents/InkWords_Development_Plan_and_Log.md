...
- [x] **任务 5**：优化 Git Clone 导致的假死与大模型生成排队（超长等待）问题。
  - **描述**：原先使用 `git clone` 拉取代码是同步操作且未禁用交互式终端提示（导致需要凭据时无限假死），现已加入 `GIT_TERMINAL_PROMPT=0` 与浅克隆 `--depth=1 --single-branch`，并增加了细粒度的环境准备状态回传；同时降低了默认并发请求（从 50 降至 3），避免向 DeepSeek API 提交并发洪峰导致的 429 错误和长达二十分钟的排队延迟。
...
<<<<<<< agent/manual-editor-entry
### [2026-04-29] Feature - 手写博客入口（创建草稿并进入编辑器）
- **开发模块**: [BlogAPI, 前端 Sidebar, Zustand Store, 文档同步]
- **完成事项**:
  1. **新增草稿创建接口**：后端新增 `POST /api/v1/blogs/draft`，创建一条顶级博客记录（`source_type=manual`）用于手写写作。
  2. **新增前端入口**：侧边栏顶部新增“写博客”按钮，点击后创建草稿并自动选中进入现有双栏编辑器。
  3. **登录过期体验优化**：前端请求遇到 401 时清理 token 并回到登录页，避免卡死在过期登录态。
  4. **文档强同步**：同步更新 API/架构/PRD/数据库/对话日志等基准文档。
- **验证**:
  - `go test ./...` 全通过
  - `docker compose down && docker compose up -d --build` 后在 `http://localhost` 点击“写博客”可创建草稿并自动保存
=======
...
- **完成事项**:
  1. 修改了 `MarkdownEngine.tsx` 中的代码块渲染逻辑。对于没有带语言标记（如 `sh`, `js`）的 ```` ` 代码块，之前会因为退化为 `<div><code>` 而导致丢失 `white-space: pre`，造成换行全部折叠。
  2. 现在，通过判断 `children` 中是否包含 `\n` 换行符来识别其为块级代码，如果是，则即使未指定语言，也回落给 `react-syntax-highlighter` 并指定语言为 `text`。
  3. 移除了 `pre` 渲染器上的自定义 `whitespace-pre` 避免对其他内置代码高亮造成双层框影响。
- **验证**: 重新编译前端镜像并在容器内启动，验证像 FFmpeg 这种多行目录结构的无语言代码块在右侧预览中能够完美保留缩进和换行展示。

### [2026-04-26] Feature - 智能推断仓库目录描述
- **开发模块**: [项目扫描接口, LLM 提取, Git API]
- **完成事项**:
  1. **README 读取**: 在 `DecompositionService.ScanProjectModulesWithProgress` 中补充了尝试通过 GitHub API 或 Git 本地命令读取仓库根目录 `README` 文件的逻辑。
  2. **智能描述生成**: 利用大模型能力，结合项目 `README` 的上下文与常见的开源目录结构规范，推断每个根目录的作用，并生成简短的中文描述（10-20字），取代了原先毫无语义的硬编码文本。
  3. **单元测试修复**: 修正了受函数签名变化影响的测试用例，确保 CI 流程畅通。
- **验证**: 通过单元测试与手动重启容器验证前端在输入大型仓库（如 FFmpeg）时能清晰显示每个目录的实际用途。
- **踩坑记录 / 架构调整**:
  - 在 Go 语言通过 `reflect` 检查方法时，`m.Type.NumIn()` 返回的参数数量包含接收者（receiver），因此在校验带有接收者的方法签名时需将数量加一。

### [2026-04-26] Bugfix & Feature - 修复思考过程泄漏、硬编码标题与稳定生成篇数
- **开发模块**: [后端 LLM 客户端, 大纲生成 Prompt, 前端状态提取]
- **完成事项**:
  1. **剥离思考过程**: 在 `backend/internal/llm/deepseek.go` 的流式与非流式方法中，分别拦截并移除了大模型的 `<think>...</think>` 思考过程，避免其污染最终生成的博客正文。
  2. **动态解析系列标题**: 修复了前端 `useFileParser.ts` 和 `useProjectAnalyzer.ts` 错误提取大纲 JSON 字段的问题，使得左侧历史记录文件夹能正确显示 AI 生成的精准 `series_title`，而非硬编码的“Git源码解析系列”。
  3. **稳定 JSON 输出**: 为 `GenerateJSON` 方法显式指定了 `Temperature: 0.1`，极大降低了生成结构化大纲时的随机性。
  4. **动态规划章节数量**: 修改了 `decomposition_generate.go` 和 `decomposition_analyze.go` 的 Prompt，取消了“不设上限”的指令。要求模型根据项目复杂度动态规划（如 1-3 篇、5-8 篇），并在 Map-Reduce 提取摘要阶段强制要求忽略细枝末节的辅助函数，防止摘要膨胀导致水文泛滥。
- **踩坑记录 / 架构调整**:
  - **Temperature 调优**: 对于结构化输出（JSON 模式），极低的 Temperature (0.1) 是保障输出稳定性的最佳实践；而对于博客正文生成，可以保持默认或稍高的温度以增加文采。
  - **Prompt 膨胀问题**: 在多级 Map-Reduce 架构中，如果下层（Map阶段）不加节制地提取细节，上层（Reduce阶段）就会认为项目极其庞大，进而规划出过多的章节。必须在最底层的 Prompt 中就强行规定“忽略琐碎信息”。

### [2026-04-26] Feature - 优化 Git Clone 导致的假死与并发排队延迟
- **开发模块**: [后端并发调度, 大模型 API 限流, 前端 UI 进度]
- **完成事项**:
  1. **禁止 Git 交互式提示**: 在 `os/exec` 调用 `git clone` 时加入 `GIT_TERMINAL_PROMPT=0` 环境变量，防止遇到需要密码的私有仓库或不存在的仓库时，后台子进程无限挂起。
  2. **浅克隆提速**: 加入 `--single-branch` 参数进一步减少下载体积，加快环境准备速度。
  3. **实时准备进度推送**: 在 `GenerateSeries` 进入 `git clone` 环节和数据库清理环节之前，通过向 `progressChan` 发送带有 `status: "progress"` 的 JSON 数据，前端随之将 `准备生成环境...` 替换为具体的 `正在克隆 GitHub 仓库代码...` 等文字。
  4. **稳健并发控制**: 调整 `maxWorkersFromEnv` 默认并发从 50 降低到 3。DeepSeek API 在面对 50 个并发流请求时极易触发限流或导致排队拥塞，造成单篇响应时间从十几秒被拖长至二十分钟以上。改用细水长流的方式能彻底解决该问题。
- **踩坑记录 / 架构调整**:
...
- **踩坑记录 / 架构调整**:
  ...
  - **Docker Compose 环境读取**: Docker Compose 默认在运行目录下寻找 `.env`，如果真正的环境变量在 `backend/.env` 中且根目录下有冲突的配置，会导致环境变量读取失败。解决方案是统一维护 `backend/.env` 并调整 `docker-compose.yml` 的挂载 fallback 逻辑。
...
### [2026-04-26] Feature - 优化大型项目大纲生成的篇数限制
- **开发模块**: [后端大模型 Prompt]
- **完成事项**:
  1. **放宽生成篇数**: 在 `DecompositionService.GenerateOutline` 中，修改了生成系列博客大纲的 Prompt，明确指示大模型“对于每个核心模块、业务逻辑或重要架构层，**都必须对应至少一篇博客进行详细说明**。不要担心生成的篇数过多！”
  2. **保留底层细节**: 修改了 `mapReduceAnalyze` 与 `generateLocalSummaryWithRetry` 等中间层局部摘要提取阶段的 Prompt，不再要求模型强制“丢弃琐碎、过度精简”，而是转为“尽可能保留各个核心模块、类、或关键函数的细节，不要过度精简导致后续无法为它们单独生成博客”。
- **验证**: 重启容器后，对于类似 FFmpeg 这样体量庞大、模块众多的仓库，系统现在将能够拆分出数十篇博客（而不是局限于原本 10 篇左右的上限），做到对核心源码文件的全面覆盖。
