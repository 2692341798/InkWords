...
3. **层级汇总 (Reduce 阶段)**：所有局部摘要拼接后，与全局目录结构合并，发起一次全局大纲规划请求。**关键优化**：此时大模型不仅会生成系列标题、章节的标题和摘要，还必须严格根据目录树输出该章节强相关的**文件路径列表 (`files` 数组)**。前端会展示 AI 提取的系列标题，允许用户确认或修改后再进行生成。
4. **内容生成 (精准按需读取 + 串行打字机渲染)**：
   - 大纲确认后，前端携带 `outline` 数组及 Git 仓库地址请求 `POST /api/v1/stream/generate`。
...
5. **断点保存与联动状态流**：每生成完其中一篇文章，立即落库并带有相同的 `ParentID` 与各自的 `ChapterSort`。前端实时更新该章节的 UI 状态（`generating` -> `completed`）。如果在生成过程中用户切换页面，前端的 `StreamStore` 会在后台**静默保持 SSE 连接**，文章依然会在后台安稳生成并落库，切回页面时进度无缝衔接。
