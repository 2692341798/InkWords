...
5. **断点保存与联动状态流**：每生成完其中一篇文章，立即落库并带有相同的 `ParentID` 与各自的 `ChapterSort`。前端右侧面板实时呈现当前正在生成的章节及总体进度，并更新该章节的 UI 状态（`generating` -> `completed`）。如果在生成过程中用户点击“停止生成”或发生异常中断，前端会保留未完成的大纲列表。用户再次点击“继续生成”时，前端仅将剩余的 `pending` 章节及 `ParentID` 传给后端，后端会自动跳过已完成章节，无缝接续同一个系列的生成，避免出现重复父节点或数据孤岛。
