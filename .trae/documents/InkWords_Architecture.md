...
- **特大型项目保护 (Map-Reduce)**:
  - **Map 阶段**: 按目录分块(针对 Git 仓库)或按字数智能段落分块(针对大于 50,000 字符的长文本文件)并发提炼局部摘要，当遇到 LLM 限流时启用带随机抖动的**指数退避 (Exponential Backoff)**。
...
1. **模块扫描与卡片选择**：针对 Git 源码，使用自定义 Writer 拦截 Git Clone 的 `stderr` 输出并通过 SSE 实时回传拉取进度；先扫描一级核心代码目录，并发调用 LLM 生成轻量级简介卡片供用户多选，实现按需定向解析。
