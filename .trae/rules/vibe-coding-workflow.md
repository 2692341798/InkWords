...
- **Token 长度与并发保护**：DeepSeek 大模型上下文上限为 1M Token (约 3,000,000 字符)。在将长文本传给模型前，**必须执行字符截断保护**（如限制最多 15,000,000 字符），防止触发 `invalid_request_error` 错误。同时需要注意由于开启了思考模式（Thinking Mode），模型的单次最大输出（含思维链）为 64K 或更高，需要对最终回答的字符和长文本结构进行优化处理。
...
## 5. 工程化与模块化约束 (高内聚低耦合)
- **单文件行数警戒线**：严格执行 **500 - 800 行** 警戒标准。当单个文件（无论是前端组件、Hooks，还是后端 Service、Handler）超过 500 行时，必须主动提出拆分重构计划；超过 800 行强制拦截并拒绝在此文件继续堆砌代码。
- **后端架构风格 (DDD / 垂直切片)**：
  - 弃用传统的按技术分层（如统一的 `controllers/`, `services/` 目录）。
  - 强制采用按业务领域组织的垂直切片（例如 `internal/domain/user/` 内部包含自身的 `handler.go`, `service.go`, `repository.go`），实现极高的内聚性。
  - 不同领域间必须通过定义明确的 Interface 接口层通信，严禁跨领域直接操作数据库或强耦合。
- **前端组件化粒度 (中等粒度)**：
  - 采用 **中等粒度** 的模块化拆分。
  - 优先提取**页面级组件**和**通用基础 UI 组件**。
  - 允许业务逻辑和局部 UI 适度绑定以兼顾开发效率，但复杂的全局状态必须抽离至 `store/`，复杂的数据获取和副作用必须抽离至自定义 Hooks (`hooks/`)。
...
- **小步迭代与强制验证**：任何功能开发必须拆分为原子的、可独立验证的步骤。编写一段核心逻辑后，必须通过编写单元测试（Go 的 `_test.go`）或执行本地环境测试来验证通过，然后再进入下一步，禁止一次性生成大量未经测试的代码。**如果更改了前端或后端代码需要查看实际效果，必须优先使用 `docker compose down && docker compose up -d --build` 一键重启项目来验证功能。**
- **代码自解释与注释规范**：
