...
func (h *Handler) ExportSeriesToObsidian(c *gin.Context) {
	...
	if h.legacyExporter == nil {
		c.JSON(http.StatusInternalServerError, gin.H{"code": http.StatusInternalServerError, "message": "exporter not configured", "data": nil})
		return
	}

	if err := h.legacyExporter.ExportSeriesToObsidian(c.Request.Context(), blogID, uid); err != nil {
...
func (h *Handler) ExportSeries(c *gin.Context) {
	...
	blogs, err := h.service.GetSeriesBlogs(c.Request.Context(), blogID, uid)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"code": http.StatusInternalServerError, "message": err.Error(), "data": nil})
		return
	}
...
func (h *Handler) ExportToObsidian(c *gin.Context) {
	...
	if err := h.legacyExporter.ExportToObsidian(c.Request.Context(), blogID, uid); err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"code": http.StatusInternalServerError, "message": err.Error(), "data": nil})
		return
	}
