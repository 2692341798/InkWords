...
func buildSeriesPDFHTML(seriesTitle string, now time.Time, blogs []Blog) (string, string, error) {
	safeTitle := strings.TrimSpace(seriesTitle)
	if safeTitle == "" {
		safeTitle = "未命名系列"
	}

	filename := fmt.Sprintf("%s.pdf", sanitizeExportFileName(safeTitle))
	...
	for idx, blog := range blogs {
		...
		anchor := fmt.Sprintf("ch-%d", idx)

		var buf bytes.Buffer
		if err := seriesPDFMarkdown.Convert([]byte(blog.Content), &buf); err != nil {
			return "", "", err
		}
	...
	for _, chapter := range chapters {
		...
		out.WriteString(html.EscapeString(chapter.ID))
		out.WriteString("\">")
		out.WriteString(html.EscapeString(chapter.Title))
...
func (s *Service) ExportSeriesToPDF(ctx context.Context, blogID uuid.UUID, userID uuid.UUID) (string, string, error) {
	...
	htmlContent, filename, err := buildSeriesPDFHTML(seriesTitle, s.now(), blogs)
	if err != nil {
		return "", "", err
	}
