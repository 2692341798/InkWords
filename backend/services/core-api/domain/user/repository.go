...
type Repository interface {
	...
	ListBlogsWithTechStacks(ctx context.Context, uid uuid.UUID) ([]Blog, error)
	GetPromptSettings(ctx context.Context, uid uuid.UUID) (*UserPromptSettings, error)
...
func (r *GormRepository) GetUserByID(ctx context.Context, uid uuid.UUID) (*User, error) {
	var user User
...
func (r *GormRepository) UpdateUsername(ctx context.Context, uid uuid.UUID, username string) error {
	return r.db.WithContext(ctx).Model(&User{}).Where("id = ?", uid).Update("username", username).Error
}

func (r *GormRepository) UpdateAvatarURL(ctx context.Context, uid uuid.UUID, avatarURL string) error {
	return r.db.WithContext(ctx).Model(&User{}).Where("id = ?", uid).Update("avatar_url", avatarURL).Error
}
...
func (r *GormRepository) ListBlogsWithTechStacks(ctx context.Context, uid uuid.UUID) ([]Blog, error) {
	var blogs []Blog
	if err := r.db.WithContext(ctx).
...
func (r *GormRepository) GetPromptSettings(ctx context.Context, uid uuid.UUID) (*UserPromptSettings, error) {
	var row UserPromptSettings
