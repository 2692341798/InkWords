...
func (s *Service) HandleCallback(ctx context.Context, provider, code string) (string, *User, error) {
	config, err := getOAuthConfig(provider)
	if err != nil {
		return "", nil, err
	}
...
func (s *Service) Login(ctx context.Context, email, password, captchaID, captchaValue string) (string, *User, error) {
...
func (s *Service) BindGithub(ctx context.Context, email, password, ghIDStr, username, avatarURL string) (string, *User, error) {
	user, err := s.repo.GetByEmail(ctx, email)
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
