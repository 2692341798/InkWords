...
// NewGeneratorService 创建 Generator 实现。
func NewGeneratorService(promptReq *PromptRequirements, persistence sharedblog.GeneratedBlogPersistence) *GeneratorService {
	return NewGeneratorServiceWithDB(nil, promptReq, persistence)
}

// NewGeneratorServiceWithDB 创建带显式 DeepSeek 客户端初始化的 Generator 实现。
func NewGeneratorServiceWithDB(db *gorm.DB, promptReq *PromptRequirements, persistence sharedblog.GeneratedBlogPersistence) *GeneratorService {
...
// GeneratePolishDraftStream 通过 LLM 流式生成本地润色草稿，不持久化。
...
func (s *GeneratorService) BuildGenerateSingleTaskResult(
	...
	sourceType string,
	content string,
) (streamdomain.GenerateSingleResult, error) {
	...
	if err != nil {
		return streamdomain.GenerateSingleResult{}, fmt.Errorf("marshal generate single task result: %w", err)
	}

	return streamdomain.GenerateSingleResult{ResultJSON: resultJSON}, nil
}
