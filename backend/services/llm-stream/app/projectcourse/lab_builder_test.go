...
func TestBuildLabManifestCreatesOrderedHashBoundCheckpoints(t *testing.T) {
	...
	require.True(t, manifest.TestFirst)
	require.Len(t, manifest.Variants, 1)
	require.NotEmpty(t, manifest.Variants[0].Tests)
...
func TestLabBuilderRejectsTraversalAndDuplicatePaths(t *testing.T) {
	_, err := BuildLabManifest("go", "1.25", []LabPatch{{Path: "../secret", Content: "x"}}, [][]LabPatch{{{Path: "main.go", Content: "x"}}}, []LabPatch{{Path: "main_test.go", Content: "x"}}, []LabPatch{{Path: "variant_test.go", Content: "x"}})
	require.ErrorContains(t, err, "unsafe lab file path")
	_, err = BuildLabManifest("go", "1.25", []LabPatch{{Path: "main.go", Content: "x"}, {Path: "main.go", Content: "y"}}, [][]LabPatch{{{Path: "main.go", Content: "z"}}}, []LabPatch{{Path: "main_test.go", Content: "x"}}, []LabPatch{{Path: "variant_test.go", Content: "x"}})
...
func TestLabManifestRejectsUnsafeCommandsAndBrokenDependencies(t *testing.T) {
	manifest, err := BuildLabManifest("go", "1.25", []LabPatch{{Path: "main.go", Content: "package main"}}, [][]LabPatch{{{Path: "main.go", Content: "x"}}}, []LabPatch{{Path: "main_test.go", Content: "x"}}, []LabPatch{{Path: "variant_test.go", Content: "x"}})
	require.NoError(t, err)
...
func TestLabBuilderRequiresIndependentVariantTests(t *testing.T) {
	_, err := BuildLabManifest("go", "1.25", []LabPatch{{Path: "main.go", Content: "package main"}}, [][]LabPatch{{{Path: "main.go", Content: "package main"}}}, []LabPatch{{Path: "main_test.go", Content: "package main"}})
	require.ErrorContains(t, err, "independent variant tests are required")
}
