...
func BuildLabManifest(language, toolchain string, starter []LabPatch, checkpointPatches [][]LabPatch, tests []LabPatch) (sharedkernel.LabManifest, error) {
	manifest := sharedkernel.LabManifest{Language: language, ToolchainVersion: toolchain, AllowedCommands: []string{"test"}, ResourceLimits: map[string]string{"timeout_seconds": "30", "memory_mb": "256", "pids": "64", "output_bytes": "1048576"}, DependencyGraph: map[string][]string{}}
	var err error
