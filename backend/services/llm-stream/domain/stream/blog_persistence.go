...
func (writableBlog) TableName() string {
	return "blogs"
}

func (b *writableBlog) BeforeCreate(tx *gorm.DB) error {
	if b.ID == uuid.Nil {
		b.ID = uuid.New()
	}
	return nil
...
type writableUserTokenBalance struct {
	ID         uuid.UUID `gorm:"type:uuid;primaryKey"`
	TokensUsed int       `gorm:"type:integer;default:0"`
}

func (writableUserTokenBalance) TableName() string {
	return "users"
}
...
// NewGeneratedBlogPersistence creates llm-stream's adapter for legacy direct
// generated-blog writes while task-only persistence is being completed.
func NewGeneratedBlogPersistence(database *gorm.DB) sharedblog.GeneratedBlogPersistence {
	return &generatedBlogPersistence{db: database}
}

func (p *generatedBlogPersistence) SaveGeneratedBlog(ctx context.Context, input sharedblog.GeneratedBlogPersistenceInput) error {
	if p.db == nil {
	...
	return p.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		tokenUpdateResult := tx.Model(&writableUserTokenBalance{}).
...
// NewContinuePersistence creates llm-stream's adapter for continue reads and
...
// NewSeriesPersistence creates llm-stream's adapter for series generation
// writes while direct blog writes are still supported.
...
func (p *seriesPersistence) EnsureSeriesParentAndDrafts(ctx context.Context, input sharedblog.SeriesDraftPreflightInput) ([]sharedblog.Chapter, error) {
	...
	if err := p.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		var existingParent writableBlog
		...
		if err := deleteQuery.Delete(&writableBlog{}).Error; err != nil {
...
func (p *seriesPersistence) SaveSeriesChapter(ctx context.Context, input sharedblog.SeriesChapterPersistenceInput) error {
	...
	return p.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		if input.BlogID != uuid.Nil {
			updateResult := tx.Model(&writableBlog{}).
...
func (p *seriesPersistence) MarkSeriesChapterFailed(ctx context.Context, userID uuid.UUID, blogID uuid.UUID) error {
	...
	return p.db.WithContext(ctx).Model(&writableBlog{}).Where("id = ? AND user_id = ?", blogID, userID).Updates(map[string]any{
		"status":  2,
		"content": "章节生成失败，请重试。",
	}).Error
}
...
func (p *seriesPersistence) SaveSeriesIntro(ctx context.Context, userID uuid.UUID, parentID uuid.UUID, content string) error {
	...
	updateResult := p.db.WithContext(ctx).Model(&writableBlog{}).Where("id = ? AND user_id = ?", parentID, userID).Updates(map[string]any{
		"content": content,
		"status":  1,
	})
	if updateResult.Error != nil {
