...
- **DeepSeek usage / 缓存命中观测基础**：系列章节终稿补强完成后，后端会额外发出 `usage` 事件，携带 `prompt_tokens / completion_tokens / prompt_cache_hit_tokens / prompt_cache_miss_tokens`，为后续前端展示“缓存命中 / 未命中”摘要和评估稳定前缀策略效果打底。
...
- **系列章节阶段可视化基础**：系列章节生成链路现在会额外发出 `understanding / drafting / reviewing / revising / streaming` 阶段事件，为后续前端章节质量进度卡打底。
- **系列章节成本可见性基础**：在 `streaming` 结束后，系列章节链路还会追加 `usage` 阶段事件，用于透传 DeepSeek usage 与 Prompt Cache 命中统计。
