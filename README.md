...
- 📦 **全能导出体系**：支持前端勾选历史博客，由浏览器离线构建 `.zip` 压缩包批量导出，或调用后端流式接口将完整项目系列一键打包下载。
- 🛡️ **极致安全与双态鉴权**：源文件解析采用“阅后即焚”策略，不在服务器进行任何物理滞留；内置常规账号密码与 GitHub OAuth2.0 一键授权双体系；集成图形验证码、邮件验证码与防爆破锁定安全机制。
