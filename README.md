...
- 📦 **全能导出与本地知识库直通**：支持前端勾选历史博客，由浏览器离线构建 `.zip` 压缩包批量导出，或调用后端流式接口将完整项目系列一键打包下载。独创 Docker Volume 挂载技术，支持将带有标准 YAML Frontmatter 的博客笔记一键导出至宿主机的本地 Obsidian Vault，实现个人知识管理 (PKM) 的无缝沉淀。
