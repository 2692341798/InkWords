...
- **知识漫游复习工作台（核心主链路）**：支持“推荐一篇 / 手动选文”，其中推荐卡默认展示今日推荐并可通过“换一篇”轮换到随机文章；同时提供 `light_recall / detailed_qa` 两种训练模式，形成中文反馈闭环与最近记录。
...
### 3.1 内容生成器交互说明
1. **选择来源**：输入 Git 仓库地址，或上传 PDF/DOCX/Markdown/TXT/ZIP 课件包。
2. **配置解析**：选择创作场景；Git 路径可进一步勾选模块，本地文件路径会先完成 parse 并停留在此步骤等待你点击“生成大纲”。
3. **确认大纲**：审阅并调整大纲；开始写作后，章节生成进度会继续保留在同一步骤内展示。
