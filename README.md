...
- 🧠 **知识漫游复习工作台**：登录后可从侧边栏进入“知识漫游复习”，支持今日推荐、随机抽题、手动选文三种入口，以及 `light_recall / detailed_qa` 两种训练模式；所有提示、追问和总结反馈均以中文返回，并保留最近复习记录。
- 🧭 **流程型首页与单步聚焦工作台**：默认工作入口升级为 `HomeEntry` 决策中心，先帮助用户在“生成博客 / 知识复习”之间做路径选择；`HomeEntry`、`Generator`、`KnowledgeReview` 统一复用共享 `StepStrip`，并确保同一时间只显示当前主步骤，减少页面“平铺感”。
